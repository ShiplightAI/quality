...
Acceptance records risk; it does not add proof or change a test result.
...
```text
We accept the missing end-to-end proof on the admin export check as tolerated
risk. Add the "missing" category to that check's accepted gaps.
```
...
underlying status and proof remain unchanged, so readers can still see what is
missing.
...
whether proof investment is being postponed indefinitely, or whether accepted
